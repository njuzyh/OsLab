...
{
	return syscall(9, flags, (uint32_t)path, 0, 0, 0);
}

int read(int fd, void *buffer, int size)
{
	return syscall(10, fd, (uint32_t)buffer, size, 0, 0);	
}

int write(int fd, void *buffer, int size)
{
	return syscall(11, fd, (uint32_t)buffer, size, 0, 0);	
}

int lseek(int fd, int offset, int whence)
{
	return syscall(12, fd, offset, whence, 0, 0);
}

int close(int fd)
{
	return syscall(13, fd, 0, 0, 0, 0);
}

int remove(char *path)
{
	return syscall(14, 0, (uint32_t)path, 0, 0, 0);
}

void ls(char *path) {
	syscall(15, 0, (uint32_t)path, 0, 0, 0);
}

void cat(char *path) {
	syscall(16, 0, (uint32_t)path, 0, 0, 0);
}
