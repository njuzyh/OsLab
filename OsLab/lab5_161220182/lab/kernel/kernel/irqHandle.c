...
void sys_read(struct TrapFrame *tf) {
        ...
        tf->eax = read(tf->ebx, (char*)tf->ecx);
}
...
void sys_fsread(struct TrapFrame *tf) {
	if(curproc->pid == 2)
                tf->ecx = tf->ecx + 0x100000;
    	uint8_t *buf = (uint8_t *)(tf->ecx);
    	int size = tf->edx;
    	fs_read(FCBlist[tf->ebx].inode, buf, size, FCBlist[tf->ebx].offset);
...
void sys_lseek(struct TrapFrame *tf) {
    	if (tf->edx == 0)
        	FCBlist[tf->ebx].offset = 0;
    	else if (tf->edx == 2)
        	FCBlist[tf->ebx].offset = fs_size("/usr/test");
    	FCBlist[tf->ebx].offset += tf->ecx;
}
...
void syscallHandle(struct TrapFrame *tf) {
	...
	switch(tf->eax) {
		...
		case 9: sys_open(tf); break;
		case 10: sys_fsread(tf); break;
                case 11: sys_fswrite(tf); break;
                case 12: sys_lseek(tf); break;
                case 13: sys_close(tf); break;
                case 14: sys_remove(tf); break;
                case 15: sys_ls(tf); break;
                case 16: sys_cat(tf); break;
	}
}
